    ...
    def run(self):
        ...
        while True:
            ...
            if distance_to_ball > approach_distance:
                ...
                angle_to_stricker = self.calculateAngle((target_x, target_y, 0), (striker_x, striker_y))
                angle_diff_to_stricker = angle_to_stricker - robot_yaw
                angle_diff_to_stricker = (angle_diff_to_stricker + math.pi) % (2 * math.pi) - math.pi
    
                if abs(angle_diff_to_stricker) > math.radians(30):
                    if angle_diff_to_stricker > 0:
                        self.startMotion(self.turnLeft30)
                    else:
                        self.startMotion(self.turnRight30)
                else:
